...
//权限定义
#define LX_PUBLIC_DPL 0

//初始化
tree_error init_lx()
{
	uint32 i;
	DiskBOOT = (_bootloder)__kmalloc(BOOTSIZE);
	if (DiskBOOT == NULL)
		return -1;
	BLOCK_SIZE=512;
...
tree_error creat_tree(int todisk)
{
	...
	case CREATE_TREE_DISK: {
							   if (_lxinfoblock->ROOTType == LX_INFO_ROOT_TYPE_LNODE) {
								   ...
								   _ln root=(_ln)lxmalloc(LXMEM_F_LNODE);
								   ...
								   _bn root=(_bn)lxmalloc(LXMEM_F_BNODE);
								   if (root == NULL)return -1;
								   _file_tree_root.node = RootAddr;
...
uint32 creatLNode()
{
	...
	_ln node =(_ln)lxmalloc(LXMEM_F_LNODE);
	if (node != NULL) {
...
uint32 creatBNode()
{
	...
	if (diskptr == (uint32)NULL)return (uint32)NULL;
	_bn node = (_bn)lxmalloc(LXMEM_F_BNODE);
	if (node != NULL) {
		saveNodePtr((Node)node, diskptr);
...
int LNode_FindFileName(_ln node, const char* str, int length)
{
	...
	if (a == NULL)return ERR_NOT_TAKE_NAME;
...
//写入文件
//path为路径+文件名
tree_error LX_FileWrite(char* filename, size_t namesize, char* data, uint64 datasize,  char hide)
{
	//直接将文件名传入节点
	if (filename[0] == '/') {
		filename++;
	...
	for (;;) {
		...
		if (err == ERR_NOT_FOUND_FILE_NAME) {//未找到文件名，需要新建,同时将上层目录的folder属性加一
			err=_folder_write(filename, fsz, LX_PUBLIC_DPL, hide);
			if (iserrcode(err))
				return err;
			//将父目录加一
	...
	if(pfi!=NULL)
		pfi->ft.folder++;
	return _file_write(filename, namesize, data, datasize, LX_PUBLIC_DPL, hide);//将文件名插入节点
}

//创建文件夹
/*方法：直接将此路径插入节点即可，如果返回ERR_SAME_FILE_NAME说明该目录已存在*/
tree_error LX_FolderWrite(char* path,size_t pathsz)
{
	if (path[0] == '/') {
		pathsz--;
		path++;
...
//删除文件或文件夹
tree_error LX_FileClear(char* filename,size_t fsz)
{
	if (filename[0] == '/')
		filename++;
	_ln node;
...
{
	sys_writeDisk(src,length,off/BLOCK_SIZE,0);
	return 0;
}
...
//读取数据
//des：目标储存，length：长度，blockaddr：块地址
uint32 readData(char* des, uint32 length, uint32 blockAddr)
{
	//asm_cpu_hlt();
	sys_readDisk(des,blockAddr*BLOCK_SIZE/512,length/BLOCK_SIZE+1,0);
	return length;
}
...
_bn diskPtr_into_BNodePtr(uint32 diskptr)
{
	...
	node =(_bn)lxmalloc(LXMEM_F_BNODE);
...
_ln diskPtr_into_LNodePtr(uint32 diskptr)
{
	...
	if (node != NULL)return node;
	node=(_ln)lxmalloc(LXMEM_F_LNODE);
	if (node == NULL)return NULL;
...
//打开文件
tree_error LX_FileOpen(const char * fname,size_t fnamesz,_ln *node)
{
	return findNode_i(fname,fnamesz,node);
}
...
//将bufsz大小的数据读入buf
//成功将会返回文件的大小
tree_error LX_FileRead(_fileitems fis,char *buf,uint bufsz)
{
	//读取数据到缓冲区
	int i,extnum=fis->ft.fatt.extname?fis->ft.extnum:1;
	long fbsz=0,bsz=bufsz,tmp,fsz=0;//fsz保存总文件大小
	for(i=0;i<extnum && bsz>0;++i){
		fbsz=fis[i].ft.size*BLOCK_SIZE+fis[i].ft.offset;//第i个描述符储存的文件大小
		fsz+=fbsz;
		if(bsz>0){//如果缓冲区剩余空间足够，继续写入
			tmp=(fbsz>bsz?bsz:fbsz);
			readData(buf,tmp,fis[i].ft.position);
			buf+=tmp;
			bsz-=tmp;
